...
public class GetParcheMembersUseCaseImpl implements GetParcheMembersUseCase {

    private final ParcheRepositorySPI parcheRepository;

    @Autowired
    public GetParcheMembersUseCaseImpl(ParcheRepositorySPI parcheRepository) {
        this.parcheRepository = parcheRepository;
    }

    @Override
    public List<Member> execute(UUID parcheId) {
        Parche parche = parcheRepository.findById(parcheId)
                .orElseThrow(() -> new RuntimeException("Parche no encontrado con id: " + parcheId));
        return parche.getMembers();
    }
}
